	    ...
	    $("#fiddadd")
	      .autocomplete({
	        ...
	        source: function( request, response ) {
	        	$.getJSON('ENS_HelpServlet?field=search_area_Equip',{
...
<%
	...
	groupBean = (ENS_AreaGroupBean) request.getAttribute("groupupdate");
	
	System.out.println("group Data :"+groupBean);
	
	ENS_AreaGroupBean groupData = null;
	if(request.getAttribute("groupupdate") != null)
	{
		System.out.println("into groupdata");
		groupData = (ENS_AreaGroupBean) request.getAttribute("groupupdate");
		//System.ot.println("sz"+ftData.size());
	}
	else
	{
		groupData = new ENS_AreaGroupBean();
		//out.println("no records to display...");	
	} 
%>
<jsp:include page="/ENS/ENS_Header.jsp"/>
<form action="ENS_AreaGroupServlet" method="get" name="formEdit">		
	...
	<div class="container mt-3">
		<div class="row">
    		...
    		<tr>
    		<tr>
    		<th>Area Name :-</font> <input type="text" name="equip_nam" id="equip_nam" value= "<%=groupData.getAREA_NAME() %>" placeholder="Equipment " readonly style="height: 50px; width: 1135px"/></p></th>
    		</tr>
			...
			<tr>    
    		<th>Equipment Name  :- </font> <input type="text" name="fiddadd" id="fiddadd" value= "<%=groupData.getEQUIP_NAME() %>" placeholder="Fidder Added " style="height: 50px; width: 1135px"/></p></th>
    		</tr>
			</table>
	</div>
</div>
